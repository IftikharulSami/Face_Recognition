...
def verify(img1_path, img2_path = '', model_name = 'VGG-Face', distance_metric = 'cosine', model = None, enforce_detection = True, detector_backend = 'opencv', align = True, prog_bar = True, normalization = 'base'):
	...
	Parameters:
		img1_path, img2_path: exact image path, numpy array or based64 encoded img could be passed. If you are going to call verify function for a list of image pairs, then you should pass an array instead of calling the function in for loops.
		...
		enforce_detection (boolean): If any face could not be detected in an image, then verify function will return exception. Set this to False not to have this exception. This might be convenient for low resolution img.
...
def analyze(img_path, actions = ['emotion', 'age', 'gender', 'race'] , models = {}, enforce_detection = True, detector_backend = 'opencv', prog_bar = True):
	...
	Parameters:
		img_path: exact image path, numpy array or base64 encoded image could be passed. If you are going to analyze lots of img, then set this to list. e.g. img_path = ['img1.jpg', 'img2.jpg']
		...
		enforce_detection (boolean): The function throws exception if a face could not be detected. Set this to True if you don't want to get exception. This might be convenient for low resolution img.
	...
	for j in global_pbar:
		...
		for index in pbar:
			...
			if action == 'emotion':
				...
				if img_224 is None:
					img_224, region = functions.preprocess_face(img = img_path, target_size = (224, 224), grayscale = False, enforce_detection = enforce_detection, detector_backend = detector_backend, return_region = True) #just emotion model expects grayscale img
...
def find(img_path, db_path, model_name ='VGG-Face', distance_metric = 'cosine', model = None, enforce_detection = True, detector_backend = 'opencv', align = True, prog_bar = True, normalization = 'base'):
	...
	Returns:
		This function returns pandas data frame. If a list of img is passed to img_path, then it will return list of pandas data frame.
	"""
	...
	if os.path.isdir(db_path) == True:
		...
		if path.exists(db_path+"/"+file_name):
			...
			print("WARNING: Representations for img in ",db_path," folder were previously stored in ", file_name, ". If you added new instances after this file creation, then please delete this file and call find function again. It will create it again.")
			...
			#------------------------
			#find representations for db img
...
def represent(img_path, model_name = 'VGG-Face', model = None, enforce_detection = True, detector_backend = 'opencv', align = True, normalization = 'base'):
	...
	"""
	This function represents facial img as vectors.

	Parameters:
		img_path: exact image path, numpy array or based64 encoded img could be passed.
